  ...
  if (isCheckingAuth) {
    return (
      <div className="text-center py-8">
        <FontAwesomeIcon icon={faSpinner} className="inline mr-2" spin />
    ...
    <form onSubmit={handleSubmit} className="space-y-6 font-inter">
      <div>
        ...
        <div className="relative">
          ...
          />
          <SimpleIcon src="/user.svg" alt="user icon" className="absolute top-3 left-3 text-gray-500" />
          {!!login.length && <SimpleIcon src="/cross.svg" alt="clear user" className="absolute top-3 right-3 text-gray-500 c cursor-pointer" onClick={() => setLogin("")} />}
        </div>
      </div>
      ...
      <div>
        ...
        <div className="relative">
          ...
          />
          <SimpleIcon src="/lock.svg" alt="password icon" className="absolute top-3 left-3 text-gray-500" />
          <SimpleIcon src="/eye-off.svg" alt="show password icon" className="absolute top-3 right-3 text-gray-500" />
        </div>
      </div>

      <div className="flex items-center">
        <input type="checkbox" id="rememberMe" checked={rememberMe} onChange={handleRememberMeChange} className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500 cursor-pointer" />
      ...
      <button type="submit" disabled={isSubmitDisabled} className="w-full bg-blue-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-blue-700 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:bg-gray-200 disabled:cursor-not-allowed transition">
      ...
      <div className="text-[18px] text-center mt-8 text-gray-600">
        Нет аккаунта?{" "}
        <a href="/" className="ml-2 font-bold underline underline-offset-4 hover:no-underline">
          Создать
        </a>
      </div>
    </form>
